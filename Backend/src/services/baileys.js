...
class BaileysService {
  ...
  handleConnectionOpen() {
    ...
    // Setup group metadata tracking
    this.setupGroupMetadataTracking();
    
    logger.info('✅ WhatsApp connection established with read receipt and group metadata tracking enabled');
  ...
  // Basic send message without reply
  async sendMessage(jid, text) {
    ...
    try {
      logger.info('Sending WhatsApp message', { jid, text: text.substring(0, 50) });
      ...
      logger.error('Failed to send message', { error: error.message, jid });
  ...
  async sendReply(jid, text, replyToMessageId = null) {
    ...
    try {
      ...
      if (replyToMessageId) {
        ...
        try {
          // Import supabase chatRepository to fetch the full raw message
          const { supabaseChatRepository } = await import('../repositories/supabase-chatRepository.js');
          const quotedMessageFromDB = await supabaseChatRepository.getMessageById(replyToMessageId);
          
          logger.info('Quoted message fetch result:', { 
            found: !!quotedMessageFromDB,
            hasRawMessage: !!quotedMessageFromDB?.raw_message,
          ...
          if (quotedMessageFromDB?.raw_message) {
            ...
            logger.warn('No raw message found for quoting, sending without quote', {
              messageExists: !!quotedMessageFromDB,
              hasRawMessage: !!quotedMessageFromDB?.raw_message
            });
          }
        } catch (dbError) {
  ...
  // Listen for group metadata updates (name changes, participant changes, etc.)
  setupGroupMetadataTracking() {
    if (!this.sock) return;
    ...
    this.sock.ev.on('groups.update', async (updates) => {
      for (const update of updates) {
        if (update.subject) {
          ...
          logger.info('[WhatsApp] Group name updated:', { 
            groupJid, 
            newName,
          ...
          await this.updateGroupNameInDB(groupJid, newName);

          // Emit to frontend for real-time UI updates
          if (typeof global !== 'undefined' && global.io) {
            global.io.emit('group_name_updated', {
              jid: groupJid,
              name: newName,
              timestamp: Date.now()
            });

            logger.info('[WhatsApp] Group name update emitted to frontend:', {
              groupJid,
              newName,
              room: 'group_name_updated'
            });
          }
        }
      }
  ...
  // Update group name in database
  async updateGroupNameInDB(groupJid, newName) {
    try {
      const { supabaseChatRepository } = await import('../repositories/supabase-chatRepository.js');
      
      const result = await supabaseChatRepository.updateGroupName(groupJid, newName);
      
      if (result) {
        logger.info('[WhatsApp] Group name updated in database:', {
          groupJid,
          newName,
          success: true
        });
      } else {
        logger.error('[WhatsApp] Failed to update group name in database:', {
          groupJid,
          newName
        });
      }
    } catch (error) {
      logger.error('[WhatsApp] Error updating group name in database:', {
        groupJid,
        newName,
        error: error.message
      });
